...
- **The append lookahead tolerates a trailing line comment on the last entry.** `(app) => AppServiceProvider(app), // core` defeated it, and the injection then fell through to the fallback and landed at the top of the list: correct Dart, wrong position.
