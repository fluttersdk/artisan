...
- **A pattern injection is checked BEFORE anything is staged, so a miss costs nothing.** Returning the `Error` from inside the stage loop was not enough: helper-backed ops write through `dart:io` during staging and sit outside the `.tmp` rollback, and the install record `plugin:uninstall` reads is only written in phase 6. So a miss found mid-loop stranded an orphan import, and possibly a pubspec entry, with nothing recorded to reverse them. `_assertPatternsResolvable` runs over the same files first and names every offending op rather than the first. An idempotent re-run still commits, because the preflight treats "the code is already there" as resolvable.

- **`InjectBeforePattern` and `InjectAfterPattern` take an optional `fallbackPattern`, tried only when the primary matches nothing.** Without it, converting the silent no-op into a hard failure widened the blast radius rather than narrowing it: a host whose `'providers': []` or `configFactories: []` is empty matched nothing before and installed quietly, and would now fail outright. The fallback anchors on the opening bracket, which is the same two-step `make:command` already does by hand (`make_command_command.dart:215-228`). One regex with an alternation cannot express it, because `firstMatch` scans by position and the opening bracket always precedes the last entry, so every injection would land at the top of a populated list.

- **`injectConfigFactory`'s regex no longer requires the entry to end in `Config,`.** A host is free to name its own factory, and `() => appSettings,` is a legal entry the old pattern refused. Widening `injectProvider` alone would have left this one silently broken and then loudly broken.

- **Both composite injections now match from their list's own key forward, so the insertion point cannot land in a different list.** Dropping the `Config,` requirement removed the only thing tying `injectConfigFactory`'s primary to `configFactories`: `() => \w+,` before a `]` describes any zero-argument closure that is last in any list, and `firstMatch` scans by position, so an earlier list anywhere in `lib/main.dart` won and the factory was appended to THAT list. An empty `configFactories: []` was the same defect wearing a second face, and the worse one: the primary matched a later list's last entry, so the fallback that exists for the empty case never ran. `'providers': [` and `configFactories: [` now open both patterns and `]` is excluded from the gap, which keeps the scan inside the list the anchor opened and turns an empty list back into the non-match the fallback is waiting for. `injectProvider` takes the same anchor, where `\w+ServiceProvider(app),` had been an accidental guard rather than a deliberate one.

- **The append lookahead skips a trailing or own-line `//` comment between the last entry and the closing bracket.** A comment trailing the entry on the same line (`(app) => AppServiceProvider(app), // core`) defeated it, and so did a comment on its own line below, which is the shape a scaffold placeholder takes (`// add plugin providers here`). Either one sent the injection to the fallback, which prepends: correct Dart, wrong position. A `/* block */` comment in that position is still a miss and still prepends, unchanged from before. It cannot skip a real entry, because every line it consumes has to be whitespace or a `//` comment through to the newline, so the append still pins to the LAST entry.

- **The providers key may be written with either quote.** Anchoring the primary on `'providers'` made its quoting load-bearing for the first time, so a host writing `"providers": [` had BOTH patterns miss and the preflight fail an install that worked before the anchor. `prefer_single_quotes` makes that shape uncommon rather than impossible, and a character class costs one character in each pattern.

- **`injectProvider` matches both spellings of a providers-list entry.** The parameter's type is optional in the pattern now, so `(app) => XServiceProvider(app),` and `(MagicApp app) => XServiceProvider(app),` both match. The lookahead that anchors the append to the LAST entry before `]` is asserted separately, because widening a pattern is the easy way to lose it. That assertion is now on where the match ENDS rather than on what it spans, since an anchored pattern necessarily carries every earlier entry in its match text while `match.end` stays the byte the code is written at. (`lib/src/helpers/config_editor.dart`, `lib/src/installer/install_operation.dart`, `lib/src/installer/install_transaction.dart`, `lib/src/installer/plugin_installer.dart`, `test/helpers/config_editor_match_report_test.dart`, `test/installer/inject_provider_shapes_test.dart`, `doc/plugins/installer-dsl.md`, `skills/fluttersdk-artisan/references/cli-commands.md`)
...
- **`XcodeProjectEditor.setEntitlementsPaths(path, {'Release': '...'})`, the per-configuration twin of `setEntitlementsPath`.** The existing setter writes one `CODE_SIGN_ENTITLEMENTS` value to every build configuration of the application target, which is right for every entitlement but one: Apple makes `aps-environment` a property of the BUILD, so a development provisioning profile carries `development` and a distribution one carries `production`, and one file cannot satisfy both. A plugin that installs push therefore had no way to leave a project able to archive, and `magic_notifications` defers the distribution value in a comment for exactly this reason. Configurations the map does not name are left untouched, so a caller can repoint Release without touching Debug, and everything that makes the original safe is shared rather than duplicated: the byte-for-byte round-trip guard, the application-target scoping that keeps the test bundle and the project defaults clean, the no-repointing rule and the staged write. The old entry point is now that shared body with a resolver answering the same path for every configuration, so the two cannot drift.
  ...
  Two refusals, because the return type has no channel for either. An EMPTY map throws `ArgumentError`: a silent no-op is indistinguishable from a successful write. A map naming configurations the project does not have throws `StateError`, which is the same "nothing will be written" condition one step later and the one a Flutter flavour produces by default, since its configurations are `Release-production` and `Release-staging` and asking for `Release` matches nothing. A PARTIAL miss stays quiet and writes what it can.

- **`start --flutter-arg=<arg>`, repeatable, forwards an argument verbatim to `flutter run`.** `start` assembled its argv from a fixed literal, so a caller who needed `--dart-define`, `--flavor`, `--web-renderer` or any other `flutter run` flag had no way to ask for it, and the only workaround was to stop using `start` and lose the session record with it: `state.json` is what gives `stop`, `status`, `hot-restart` and every MCP tool something to act on, and nothing else writes it. Reported from a consumer that wanted a compile-time define to switch a fixture size for a performance run and had to move the switch onto a URL query parameter instead. The values land after the arguments `start` builds, so a repeated flag overrides the default it chose (`flutter run` takes the last occurrence), they are recorded in `state.json` as `flutterArgs`, and `restart` replays them. `restart` declares the same flag, so an explicit one wins over the carried set exactly as `--device` and the ports already do. The comma split `args` applies to a multi-option by default is switched off: it is the wrong shape for a passthrough and it broke the exact case the flag exists for, turning `--flutter-arg=--dart-define=TAGS=a,b` into two tokens so that `flutter run` read the bare `b` as a positional target. Carrying them matters more than carrying a port: a dropped port refuses to bind and says so, while a dropped define compiles clean and the app behaves differently.

### Changed

- **`_buildSettingsOf` now throws when a build configuration carries no `name`.** It has to read the name to let a caller address one configuration, and a configuration nothing can address is a project this should report rather than quietly halve. `InstallTransaction._pointXcodeAtEntitlements` already catches `StateError` and degrades to a warning, so an install skips the setting rather than aborting.
