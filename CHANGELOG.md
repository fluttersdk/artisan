...
- **The append lookahead skips whatever separates the last entry from the closing bracket.** A comment trailing the entry on the same line (`(app) => AppServiceProvider(app), // core`) defeated it, and so did a comment on its own line below, which is the shape a scaffold placeholder takes (`// add plugin providers here`). Either one sent the injection to the fallback, which prepends: correct Dart, wrong position. It cannot skip a real entry, because every line it consumes has to be whitespace or a `//` comment through to the newline, so the append still pins to the LAST entry.
