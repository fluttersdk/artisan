...
Remote HTTP transport is NOT supported in V1; the artisan MCP server is stdio-only because it bridges to the local VM Service URI recorded in this project's session by `./bin/fsa start` (or `dart run :dispatcher start` when `bin/fsa` is absent).
