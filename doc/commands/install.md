...
Run `dart pub get` (or `flutter pub get`) after `install` completes to resolve to a concrete version. Tighten the constraint by hand (e.g. `^0.0.15`) once the consumer has chosen a baseline.
...
```
  ...
  inject  pubspec.yaml (fluttersdk_artisan: ^0.0.15)
