...
void main() {
  ...
  group('MakePluginCommand — generic-mode scaffolding', () {
    ...
    test('overwrites flutter create defaults with our pubspec + lib + test',
      ...
      // pubspec must be our generic version (has executables: artisan:), NOT
  ...
  group('MakePluginCommand, magic mode shape matches magic_logger reference',
    ...
    test(
        'bin/artisan.dart IS generated by make:plugin '
        '(plugin self-bootstrap entry: `dart run <plugin>:artisan <cmd>`)',
        () async {
      final exit = await runMagicScaffold('magic_logger');
      expect(exit, 0);

      // Wave 4 E2E surfaced that `dart run <plugin>:artisan` requires
      // bin/artisan.dart to exist (per pubspec executables: field). The
      ...
      final binFile = File(p.join(target.path, 'bin', 'artisan.dart'));
